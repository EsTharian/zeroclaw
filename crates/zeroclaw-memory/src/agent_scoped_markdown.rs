...
impl Memory for AgentScopedMarkdownMemory {
    ...
    async fn recall(
        ...
        let mut merged = Self::attribute(
            &self.own_alias,
            self.own
                .recall(query, limit, session_id, since, until, search_mode.clone())
                .await?,
        );
    ...
    async fn recall_for_agents(
        ...
        if allowed_agent_ids.is_empty() {
            return self.recall(query, limit, session_id, since, until, None).await;
        }
...
mod tests {
    ...
    async fn store_writes_only_to_own_backend() {
        ...
        let hits = scoped
            .recall("alpha-only", 10, None, None, None, None)
            .await
            .unwrap();
    ...
    async fn recall_unions_own_and_peer_rows_with_attribution() {
        ...
        let hits = scoped.recall("shared", 10, None, None, None, None).await.unwrap();
    ...
    async fn recall_attribution_carries_through_both_key_prefix_and_alias_field() {
        ...
        let hits = scoped.recall("from", 10, None, None, None, None).await.unwrap();
