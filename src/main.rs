        ...
        Commands::Gateway { gateway_command } => {
            match gateway_command {
                Some(zeroclaw::GatewayCommands::Restart { port, host }) => {
                    ...
                    let restart_tls_enabled =
                        config.gateway.tls.as_ref().is_some_and(|t| t.enabled);
                    match shutdown_gateway(
                        &host,
                        port,
...
async fn shutdown_gateway(
    ...
    let url = gateway_admin_url(host, port, path_prefix, "/admin/shutdown", tls_enabled);
...
async fn fetch_paircode(
    ...
    let response = if new {
        ...
        let url = gateway_admin_url(host, port, path_prefix, "/admin/paircode/new", tls_enabled);
        client
            .post(&url)
            .timeout(std::time::Duration::from_secs(5))
            .send()
        ...
        let url = gateway_admin_url(host, port, path_prefix, "/admin/paircode", tls_enabled);
        client
            .get(&url)
            .timeout(std::time::Duration::from_secs(5))
            .send()
...
fn gateway_admin_url(
    ...
    format!("{scheme}://{host}:{port}{prefix}{admin_path}")
}
...
mod tests {
    ...
    fn gateway_admin_url_uses_unprefixed_admin_path_by_default() {
        assert_eq!(
            gateway_admin_url("127.0.0.1", 42617, None, "/admin/paircode", false),
    ...
    fn gateway_admin_url_prepends_configured_path_prefix() {
        assert_eq!(
            gateway_admin_url("localhost", 42617, Some("/zeroclaw"), "/admin/paircode/new", false),
    ...
    #[test]
    #[cfg(feature = "agent-runtime")]
    fn gateway_admin_url_uses_https_when_tls_enabled() {
        assert_eq!(
            gateway_admin_url("localhost", 42617, None, "/admin/paircode", true),
            "https://localhost:42617/admin/paircode"
        );
    }
    ...
    fn paircode_url_combines_host_port_override_with_configured_path_prefix() {
        ...
        assert_eq!(
            gateway_admin_url("192.168.1.20", 42617, Some("/gw"), "/admin/paircode", false),
