...
async fn compare_recall_quality() {
    ...
    for (query, desc) in &queries {
        let sq_results = sq.recall(query, 10, None, None, None, None).await.unwrap();
        let md_results = md.recall(query, 10, None, None, None, None).await.unwrap();
...
async fn compare_recall_speed() {
    ...
    let start = Instant::now();
    let sq_results = sq
        .recall("Rust systems", 10, None, None, None, None)
        .await
        .unwrap();
    let sq_dur = start.elapsed();
...
async fn compare_upsert() {
    ...
    let md_results = md.recall("loves Rust", 5, None, None, None, None).await.unwrap();
